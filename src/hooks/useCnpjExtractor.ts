...
export function useCnpjExtractor() {
  ...
  const loadMunicipios = useCallback(async (uf: string) => {
    ...
    try {
      const { data } = await supabase.functions.invoke("cnpj-extractor", {
        body: { action: "load-municipios", uf },
      });
      if (data?.municipios) {
      ...
      try {
        while (page <= maxPages && hasMore) {
          ...
          setState((s) => ({
            ...
            progress: Math.round((page / maxPages) * 100),
            total,
            currentPage: page,
          }));
        ...
        toast({
          ...
          description: `${allCompanies.length} empresas encontradas.`,
        });
      } catch (err: unknown) {
  ...
  const exportCsv = useCallback(() => {
    ...
    const headers = [
      ...
      "CNAE", "Código CNAE", "Logradouro", "Número", "Bairro",
      "Município", "UF", "CEP", "Telefone 1", "Telefone 2",
      "Email", "Capital Social", "Porte", "Natureza Jurídica", "Sócios",
    ];

    const rows = state.companies.map((c) => [
      c.cnpj, c.razao_social, c.nome_fantasia, c.situacao_cadastral,
      c.data_abertura, c.cnae_principal, c.cnae_codigo, c.logradouro,
      c.numero, c.bairro, c.municipio, c.uf, c.cep,
      c.telefone1, c.telefone2, c.email, c.capital_social, c.porte,
      c.natureza_juridica, c.socios,
    ]);
    ...
    a.href = url;
    a.download = `cnpj_extrator_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
